        ...
        Some(message) => match message {
          ...
          ProtocolMessage::BlockResponse(block) => {
            ...
            match ledger.add_block_by_id(&block) {
              BlockStatus::Applied => {
                ...
                main_to_net_tx.send(ProtocolMessage::BlockRequest(ledger.height)).await;
              },
              BlockStatus::Pending => {
                // error in sequencing
          ...
          ProtocolMessage::BlockProposal(full_block) => {
            ...
            if ledger.is_block_applied(&block_id) {
              println!("Received a block proposal via gossip for known block, ignoring");
              continue;
            }
            ...
            if !full_block.is_valid(PIECE_COUNT, &ledger.merkle_root, &genesis_piece_hash) {
          ...
          ProtocolMessage::BlockSolution(solution) => {
            println!("Received a solution for challenge: {}", hex::encode(&solution.challenge));
            ...
            match ledger.add_block_by_id(&block) {
              BlockStatus::Applied => {
                ...
                let solve = main_to_sol_tx.send(ProtocolMessage::BlockChallenge(block.get_id()));
                let gossip = main_to_net_tx.send(ProtocolMessage::BlockProposal(FullBlock {
                  block,
                  proof,
                }));
  ...
  let protocol_startup = async {
    ...
    match mode {
      NodeType::Gateway => {
        ...
        println!("Starting gateway with genesis challenge: {}", hex::encode(&genesis_piece_hash));
        main_to_sol_tx.send(ProtocolMessage::BlockChallenge(genesis_piece_hash)).await;
      },
