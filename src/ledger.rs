...
impl FullBlock {
  ...
  pub fn is_valid(&self, piece_count: usize, merkle_root: &[u8], genesis_piece_hash: &[u8; 32]) -> bool {
    ...
    let decoding = crypto::decode_eight_blocks_parallel(&self.proof.encoding, &id, self.proof.piece_index as usize);
...
impl Ledger {
  ...
  pub fn cache_pending_block(&mut self, full_block: FullBlock) {
    ...
    self.pending_parents.insert(full_block.block.parent_id, block_id.clone());
    ...
    match self.pending_parents.get(&parent_id) {
      Some(child_id) => {
        match self.pending_blocks.get(child_id) {
          Some(full_block) => {
            println!("Got child full block with id: {}", hex::encode(full_block.block.get_id()));
            ...
            if !full_block.is_valid(PIECE_COUNT, &self.merkle_root, &self.genesis_piece_hash) {
          ...
          None => {
            panic!("Pending blocks are out of sync, cannot retrieve full pending block");
          }
        }
      },
      None => {
  ...
  pub fn add_block_by_id(&mut self, block: &Block) -> BlockStatus {
    ...
    if self.height > 0 {
      ...
      match self.blocks_by_id.get_mut(&block.parent_id) {
        ...
        None => {
          println!("Could not find parent block in blocks_by_id with id: {}", hex::encode(&block.parent_id));
