  ...
  async fn try_ping_all(&self, peer_data: Vec<u8>, node: &Node) {
    ...
    for peer in peer_list.peers.iter() {
      if !self.peers.lock().await.contains_key(&peer.0) {
        node.send_message(peer.1, NetworkMessageName::Ping, &[]).await;
      }
    }
  }
      ...
      while *is_connected.lock().await == false {
        ...
        if message.name == NetworkMessageName::Pong {
          *is_connected.lock().await = true;
          ...
          println!("Did not receive pong as first message! Received: {:?}", message.name);
        }
      }
      ...
      while *has_peers.lock().await == false {
        ...
        if message.name == NetworkMessageName::PeersResponse {
          *has_peers.lock().await = true;
          ...
          println!("Did not receive peers as second message! Received: {:?} ", message.name);
        }
      }
    }
    ...
    loop {
      ...
      match message.name {
        ...
        NetworkMessageName::BlockResponse => {
          ...
          any_to_main_tx.send(ProtocolMessage::BlockResponse(block)).await;
        },
        NetworkMessageName::BlockProposal => {
          ...
          any_to_main_tx.send(ProtocolMessage::BlockProposal(full_block)).await;
        },
      }
    }
  };
